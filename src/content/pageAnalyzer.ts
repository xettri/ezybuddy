...
export type PageContext = {
  ...
  markdown: string; selectionContext?: SelectionContext;
};
// Legacy compat exports
export type Section = { heading: string; text: string };
export type ContentNode = {
  level: number; heading: string; text: string;
  links: { label: string; href: string }[]; children: ContentNode[];
};

// Graph Types
type NodeKind =
  | "heading" | "paragraph" | "link" | "image"
  | "list" | "list-item" | "table" | "code"
  | "quote" | "chip" | "hr" | "root";
...
const SKIP_TREE = new Set([
  ...
  "NAV", "FOOTER", "SVG", "CANVAS", "VIDEO", "AUDIO",
  "HEAD", "META", "LINK", "BASE", "BUTTON", "SELECT",
  "FORM", "OBJECT", "EMBED"
]);
...
const NOISE_PHRASES = new Set([
  ...
  "skip to main", "skip to content", "back to top",
  "add to cart", "add to wishlist", "add to wish list",
  "buy now", "shop now", "sign in", "log in", "register",
  "cookie", "accept all", "decline",
]);
...
function trackSelection(): void {
  ...
  m.id = "eb-sel-marker"; m.style.display = "none";
  document.documentElement.appendChild(m);
  document.addEventListener("selectionchange", () => {
    const sel = window.getSelection();
    if (!sel || sel.isCollapsed) { lastSelection = undefined; return; }
    const text = sel.toString().replace(/\s+/g, " ").trim();
    if (!text) { lastSelection = undefined; return; }
    const anchor = sel.anchorNode;
    const parent = anchor?.nodeType === Node.TEXT_NODE
      ? anchor.parentElement : (anchor as Element | null);
    const ctx = ((parent as HTMLElement)?.innerText ?? "").replace(/\s+/g, " ").trim();
    const idx = ctx.indexOf(text);
    const surrounding = idx >= 0
      ? ctx.slice(Math.max(0, idx - MAX_SURROUNDING / 2), Math.min(ctx.length, idx + text.length + MAX_SURROUNDING / 2))
...
function mkNode(kind: NodeKind, text: string, parent: number, extra?: Partial<GNode>): GNode {
  ...
  const node: GNode = {
    id, kind, text: norm, hash: djb2(norm),
    children: [], parent, pruned: false, ...extra
  };
  _graph.set(id, node);
...
function walkDOM(el: Element, parentId: number): void {
  ...
  // HR
  if (tag === "HR") { mkNode("hr", "---", parentId); return; }
...
function pruneNoise(rootId: number): void {
  ...
  while (queue.length) {
    ...
    if (t && NOISE_RE.test(t)) { node.pruned = true; continue; }

    // UI noise phrases
    if (t && NOISE_PHRASES.has(t.toLowerCase())) { node.pruned = true; continue; }

    // Very short non-link, non-heading text (single words like ">", "1", "$")
    if (t && t.length < 3 && !["link", "chip", "heading", "hr", "image"].includes(node.kind)) {
      node.pruned = true; continue;
    }
  ...
  switch (node.kind) {
    ...
    case "list": {
      ...
      for (const cid of node.children) {
        ...
        nested.forEach(nl => { budget.chars -= nl.length; });
      }
    ...
    case "table": {
      ...
      const separator = "| " + rows[0].text.split(" | ").map(() => "---").join(" | ") + " |";
...
function findContentRoot(): Element {
  const selectors = [
    ...
    "#main-content", "#content", "#main", "#app", "#root",
...
export function buildPageContext(): PageContext {
  ...
  console.debug(`[EzyBuddy] graph: ${graphSize} nodes, ${pruned} pruned → ${markdown.length} chars`);
  try { (window as any).ezybuddyContext = context; } catch { }
